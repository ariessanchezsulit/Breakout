...
namespace Framework
{
    public class PopupCollectionRoot : Scene
    {
        ...
        {
            // +AS:180404 TODO
            //  - Fix Popup Sorting with SystemCanvas (Add SystemCanvas Support)
        ...
        private void SortPopupCanvas()
        {
            ...
            float depth = (float)SceneDepthType.ToInt();
            ...
            for (int i = 1; i < count; i++)
            {
                if (i == 1)
                {
                    ...
                    canvas.sortingOrder = SceneDepthType.ToInt() - 1;
                ...
                canvas = CanvasList[i];
                canvas.renderMode = RenderMode.ScreenSpaceCamera;
                canvas.sortingOrder = SceneDepthType.ToInt();
                canvas.worldCamera = camera;
            ...
            for (int i = 2; i < count; i++)
            {
                canvas = CanvasList[i];
                canvas.renderMode = RenderMode.ScreenSpaceCamera;
                canvas.sortingOrder = SceneDepthType.ToInt() - 2;
                canvas.worldCamera = camera;
                canvas.transform.SetSiblingIndex(i);
            }
            //*/
