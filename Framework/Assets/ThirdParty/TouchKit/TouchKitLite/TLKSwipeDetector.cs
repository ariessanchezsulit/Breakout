	...
	void FixedUpdate()
	{
		    ...
		    else if( touch.phase == TouchPhase.Moved )
		    {
                if (!_didCompleteDetection && checkForSwipeCompletion(touch))
                {
                    ...
                    //MessageBroker.Default.Publish<OnSwipeDetection>(new OnSwipeDetection()
                    //{
                    //    ThisSwipeDirection = completedSwipeDirection
                    //});

                    completedSwipeDirection = SwipeDirection.None;
                }
            }
            else if(touch.phase == TouchPhase.Ended)
            {
                //Debug.Log("was a tap!~");
                //MessageBroker.Default.Publish<OnTapDetection>(new OnTapDetection());
